...
let%expect_test "match tuple binding" =
  go
    {|
let _ = match (1, true) { (x, y) -> assert (x == 1) };;
|};
  [%expect {| |}]
;;
...
let%expect_test "match tuple binding second element" =
  go
    {|
let _ = match (1, 2) { (x, y) -> assert (y == 2) };;
|};
  [%expect {| |}]
;;
...
let%expect_test "match 3-element tuple" =
  go
    {|
let _ = match (1, 2, 3) { (a, b, c) -> assert (a + b + c == 6) };;
|};
  [%expect {| |}]
;;
...
let%expect_test "match nested tuple" =
  go
    {|
let _ = match ((1, 2), (3, 4)) { ((a, b), (c, d)) -> assert (a + b + c + d == 10) };;
|};
  [%expect {| |}]
;;
...
fun f (x : ((int ^ int) ^ int) ^ int) : int =
  match x { (((a, b), c), d) -> a * 1000 + b * 100 + c * 10 + d }
;;
...
fun outer (z : int) : bool -> int =
  fn (b : bool) -> match b { true | false -> z }
;;
...
let _ = match swap (10, 20) { (x, y) -> assert (x == 20) };;

let _ = match swap (10, 20) { (x, y) -> assert (y == 10) };;
|};
  [%expect {| |}]
;;
...
let%expect_test "deeply nested match on nested tuples" =
  go
    {|
let _ = match ((1, 2), (3, 4)) { ((a, b), (c, d)) -> (fn (static erased t : type) -> ()) };;
|};
  [%expect {| |}]
;;
